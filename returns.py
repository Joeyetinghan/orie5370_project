...
# Calculate predicted price and returns 
def res(stock_list, Ind, alpha=0.5, gamma=0.1, period=24):
    df1 = pd.DataFrame()
    for i in stock_list:
        name = i 
        if Ind == 'SMA':
            df1[name] = SMA_price(np.array(dff[i]), 24)
        elif Ind == 'EMA':
            df1[name] = EMA_price(np.array(dff[i]), alpha)
        elif Ind == 'EMA_p':
            df1[name] = EMA_price_new(np.array(dff[i]), alpha, period)
        elif Ind == 'AMA':
            df1[name] = AMA(dff[i])
        elif Ind == 'ALOMA':
            df1[name] = AOLMA(np.array(dff[i]), gamma)
        else:
            return "Invalid Input"
        
    return df1

# Pass in SMA/EMA/EMA_p/AMA/ALOMA
def res(stock_list, Ind, alpha=0.5, gamma=0.1, period=24):
    df1 = pd.DataFrame()
    for i in stock_list:
        name = i 
        if Ind == 'SMA':
            df1[name] = SMA_price(np.array(close_price[i]), 24)
        elif Ind == 'EMA':
            df1[name] = EMA_price(np.array(close_price[i]), alpha)
        elif Ind == 'EMA_p':
            df1[name] = EMA_price_new(np.array(close_price[i]), alpha, period)
        elif Ind == 'AMA':
            df1[name] = AMA(close_price[i])
        elif Ind == 'ALOMA':
            df1[name] = AOLMA(np.array(close_price[i]), gamma)
        else:
            return "Invalid Input"
...
# ALOMA result:
df_p = res(stock_list, 'ALOMA')
df_p
daily_returns_alo = pd.DataFrame()
for i in close_price.columns:
    temp1 = close_price[i]
    temp2 = df_p[i]
    daily_returns_alo[i] = np.array(close_price[i])/np.array(df_p[i].shift()) - 1

daily_returns_alo.index = close_price.index.values
daily_returns_alo